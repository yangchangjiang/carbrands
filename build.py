...
GA = '<script async src="https://www.googletagmanager.com/gtag/js?id=G-BD8ECK00KK"></script><script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag("js",new Date());gtag("config","G-BD8ECK00KK");</script>'
...
PRIVACY = {"en":"Privacy","zh-CN":"隐私","ja":"プライバシー","ko":"개인정보","es":"Privacidad","pt":"Privacidade","fr":"Confidentialité","de":"Datenschutz","ar":"الخصوصية","hi":"गोपनीयता","th":"ความเป็นส่วนตัว","vi":"Quyền riêng tư","id":"Privasi","ru":"Конфиденциальность"}
ABOUT = {"en":"About","zh-CN":"关于","ja":"概要","ko":"소개","es":"Acerca","pt":"Sobre","fr":"À propos","de":"Über","ar":"حول","hi":"बारे में","th":"เกี่ยวกับ","vi":"Giới thiệu","id":"Tentang","ru":"О нас"}
...
def build_brand_page(b, lang):
    ...
    article_schema = f'<script type="application/ld+json">{{"@context":"https://schema.org","@type":"Article","headline":"{b["name"]} Car Models & Specs","description":"{b["description"]}","author":{{"@type":"Organization","name":"{DOMAIN}"}}}}</script>'
    return f'<!DOCTYPE html><html lang="{lang}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{b["name"]} {ui["brands"]} — {len(b_cars)} {ui["models"]}, {ui["founded"]} {b["founded"]}, {b["types"]} | {DOMAIN}</title><meta name="description" content="{b["name"]} {ui["brands"]} — {len(b_cars)} {ui["models"]} with specs, pricing, reviews. {ui["founded"]} {b["founded"]}, {ui["country"]} {loc}. Types: {b["types"]}. {b["description"]} Find {b["name"]} car models, specs, horsepower, fuel efficiency."><meta name="keywords" content="{b["name"]}, {b["name"]} models, {b["name"]} cars, {b["name"]} specs, {b["name"]} price, {b["name"]} review, {b["name"]} {b["country"]}, {b["name"]} 2024, {b["name"]} lineup"><style>{CSS}</style>{GA}{ADS}{hreflang}</head><body><header><nav><span class="logo">🏎️ {DOMAIN}</span><a href="/{lp}">{ui["home"]}</a><a href="/{lp}about">{ABOUT.get(lang,"About")}</a><a href="/{lp}privacy">{PRIVACY.get(lang,"Privacy")}</a>{DROPDOWN}</nav></header><div class="detail-hero"><div class="brand-name-lg">{html.escape(b["name"])}</div><div class="brand-sub">{loc} · {ui["founded"]} {b["founded"]} · {len(b_cars)} {ui["models"]}</div></div><div class="model-grid">{model_cards}</div><footer>&copy; 2026 {DOMAIN}</footer><div class="cookie-consent"><span>{cookie[0]}</span><div class="cookie-btns"><button class="cookie-accent" onclick="this.parentElement.parentElement.style.display=\'none\'" style="background:var(--accent);color:white;border:none;padding:8px 24px;border-radius:4px;cursor:pointer;font-weight:600">{cookie[1]}</button><button class="cookie-decline" onclick="this.parentElement.parentElement.style.display=\'none\'" style="background:#3f3f46;color:white;border:none;padding:8px 24px;border-radius:4px;cursor:pointer">{cookie[2]}</button></div></div>{GOLANG}{BAIDU}{faq}{bc}{article_schema}</body></html>'
